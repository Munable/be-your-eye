...
class CommunityDownloadInstrumentedTest {
    ...
    fun liveDownloadRequiresConsentCancelsResumesAndReusesOffline() = runBlocking {
        ...
        fun coordinator(online: Boolean, now: Long = System.currentTimeMillis()) = ModelPreparationCoordinator(
            appContext = context, filesDir = root, catalogUrl = BuildConfig.MODEL_CATALOG_URL,
            buildChannel = BuildChannel.COMMUNITY, device = currentModelPreparationDevice(context),
        ...
        try {
            ...
            var reuseSeen = false
            val reused = coordinator(false, System.currentTimeMillis() + 30L * 24 * 60 * 60 * 1000).prepareTransientReading(UuidV7.generate(), onProgress = {
            ...
            File(context.filesDir, "download-ux-result.json").writeText(
                """{"total_bytes":${request.totalBytes},"artifact_requests":$completedRequests,"range_resume_requests":${resumes.get()},"consent_before_download":true,"cancelled":true,"injected_network_failure_retried":true,"offline_reuse":true,"directory_expired_30_days_reuse":true}""",
            )
        } finally {
